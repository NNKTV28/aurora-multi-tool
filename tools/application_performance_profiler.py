...
def application_performance_profiler():
    ...
    try:
        ...
        processes = sorted(processes, key=lambda p: p['cpu_percent'], reverse=True)[:10]  # Top 10 by CPU usage

        print(f"{'PID':<10}{'Name':<25}{'CPU%':<10}{'Memory (MB)':<15}")
        print("-" * 60)
        for proc in processes:
            print(f"{proc['pid']:<10}{proc['name']:<25}{proc['cpu_percent']:<10.2f}{proc['memory_info'].rss / (1024 * 1024):<15.2f}")
